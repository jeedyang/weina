...
void MainCtrl::on_testDone(int id)
{
	...
	for (int i = 0; i < 24; i++)
	{
		qDebug() << _tr("%1ºÅ´«¸ÐÆ÷ ·Öµµ½á¹û :").arg(i) << m_testMods[id]->result[i]<<_tr("×îÐ¡µç×èÖµ :")<< m_testMods[id]->minRes[i] << _tr("×î´óµç×èÖµ :") << m_testMods[id]->maxRes[i]<<_tr("±ÈÖµ :%1").arg(m_testMods[id]->maxminOdds[i]);
